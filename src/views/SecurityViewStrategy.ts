...
/** Backwards-compatible test/helper entry point with an explicit entity list. */
export function buildSecurityActivitySection(
  ...
  if (dashboardConfig.show_security_activity !== true) return null;
  if (!hass.config?.components?.includes('logbook')) return null;
  const visibleIds = entityIds.filter((id) => !isExcludedFromSecurityLog(id));
  if (visibleIds.length === 0) return null;
  return {
    type: 'grid',
    cards: [
      { type: 'heading', heading: localize('security.activity'), heading_style: 'title' },
      {
        type: 'logbook',
        entities: visibleIds,
        target: { entity_id: visibleIds },
        hours_to_show: 24,
...
defineViewStrategy('ll-strategy-dashboard-strategy-view-security', generateSecurityView);
