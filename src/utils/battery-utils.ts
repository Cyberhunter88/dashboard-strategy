...
type BatteryStatusConfig = Pick<
  ...
  'battery_critical_threshold' | 'battery_low_threshold' | 'hide_unavailable_entities'
>;
...
export function getBatteryStatus(
  ...
  if (isUnavailableState(state.state)) return 'critical';
  ...
  if (Number.isNaN(value)) return 'critical';
