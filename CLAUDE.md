...
- [x] Visual Card Picker in editor: clicking "+" opens a searchable card type grid (20 built-in HA types + installed custom cards from `window.customCards`); step 2 shows `hui-card-element-editor` for visual editing (with YAML textarea fallback); works for Custom Cards, Custom Sections, and Area Custom Cards; overlay dismisses on backdrop click; fully non-breaking (v1.9.0)
- [x] Codebase cleanup & performance: duplicate `declare global` blocks consolidated into `homeassistant.ts`; duplicate `SummaryType` local type removed (imported from `types/strategy`); dead Registry methods `getHiddenForGroup`/`getHiddenForAreaGroup` deleted; `new Date()` allocations in all three sort comparators replaced with direct ISO-8601 string comparison; `getCardSize()` in LightsGroupCard + CoversGroupCard changed from O(n) filter+sort to O(1) cached Set size read; `LightsGroupCard._groupByFloors` uses `Registry.areas` instead of `Object.values(hass.areas)`; `SummaryCard._getDisplayConfig()` switch/case instead of full 5-key Record allocation
